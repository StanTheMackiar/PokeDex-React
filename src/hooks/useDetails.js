...
const useDetails = (id) => {
  ...
  const getEvoData = async (name) => {
    ...
    if (!res.err) {
      ...
      const type1 = res.types[0] ? helpNamePokemons(res.types[0].type.name) : null;
      const type2 = res.types[1] ? helpNamePokemons(res.types[1].type.name) : null;
  ...
  const details = async (res, species, evo) => {
    ...
    const habitat = species.habitat ? helpNamePokemons(species.habitat.name) : null;
    ...
    const description = species.flavor_text_entries
      ...
      : null;
    const ability =
      res.abilities.length > 0 ? helpNamePokemons(res.abilities[0].ability.name) : null;
